...
def upload_file_to_bucket(job_id, file_path):
    """
    Upload any file (image, video, gif, ...) to S3/R2-compatible bucket storage
    and return a presigned URL.

    Unlike rp_upload.upload_image(), this:
      - detects the real Content-Type via mimetypes instead of assuming
        "image/<extension>" (which mislabels .mp4/.webm as image/*),
      - accepts an explicit bucket name via the BUCKET_NAME env var, since
        R2's account-level endpoint (https://<account_id>.r2.cloudflarestorage.com)
        doesn't bake the bucket name into the host the way rp_upload's AWS-style
        examples do, so its undocumented default of a month-string bucket name
        (e.g. "09-26") would silently target a bucket that doesn't exist.

    Returns None if no bucket is configured (BUCKET_ENDPOINT_URL unset or boto3
    missing) so callers can fall back to base64.
    """
    boto_client, _ = rp_upload.get_boto_client()
    if boto_client is None:
        return None

    bucket = os.environ.get("BUCKET_NAME") or time.strftime("%m-%y")
    file_extension = os.path.splitext(file_path)[1]
    content_type = mimetypes.guess_type(file_path)[0] or "application/octet-stream"
    key = f"{job_id}/{uuid.uuid4().hex[:8]}{file_extension}"
...
def handler(job):
    ...
    try:
        ...
        for node_id, node_output in outputs.items():
            ...
            # VHS_VideoCombine (and similar video-output nodes) save their result
            # under "gifs", not "images" — handled the same way as images above,
            # just via upload_file_to_bucket() so large videos don't have to be
            # returned as base64 (which silently overflows RunPod's response
            # size limit — see the "SaveImage 161 frames" test in this project's
            # history for how that failure looks: a completed job with a totally
            # empty response body).
            if "gifs" in node_output:
                print(
                    f"worker-comfyui - Node {node_id} contains {len(node_output['gifs'])} gif/video output(s)"
                )
                for gif_info in node_output["gifs"]:
                    filename = gif_info.get("filename")
                    subfolder = gif_info.get("subfolder", "")
                    file_type = gif_info.get("type")
                        ...
                        if os.environ.get("BUCKET_ENDPOINT_URL"):
                            ...
                            try:
                                with tempfile.NamedTemporaryFile(
                                    ...
                                    temp_file_path = temp_file.name
                                print(
                                    f"worker-comfyui - Uploading {filename} to bucket..."
                                )
                                bucket_url = upload_file_to_bucket(
                                    job_id, temp_file_path
                                )
                                if not bucket_url:
                                    raise RuntimeError(
                                        "boto3 client unavailable — check BUCKET_* env vars"
                                    )
                                print(
                                    f"worker-comfyui - Uploaded {filename}: {bucket_url}"
                                )
                                output_data.append(
                                    {
                                        "filename": filename,
                                ...
                                error_msg = f"Error uploading {filename} to bucket: {e}"
                                print(f"worker-comfyui - {error_msg}")
                                errors.append(error_msg)
                            finally:
                            ...
                            # No bucket configured — fall back to base64, same as
                            # images. Fine for short clips, but this is exactly
                            # the path that overflowed the response before.
                            try:
                                ...
                                print(
                                    f"worker-comfyui - Encoded {filename} as base64 ({len(base64_data)} chars)"
                                )
                            except Exception as e:
                                error_msg = f"Error encoding {filename} to base64: {e}"
                                print(f"worker-comfyui - {error_msg}")
